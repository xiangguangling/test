...
export default function RegionalPage({ data }: { data: DashboardData }) {
  ...
  useEffect(() => {
    ...
    keys.forEach((k, xi) => areas.forEach((a, yi) => { heatData.push([yi, xi, +(data.urban_rural_analysis[a]?.[k] ?? 0).toFixed(3)]); }));
    const chart = mountEcharts(heatRef.current, { tooltip: { formatter: (p: any) => `${areas[p.value[0]]} · ${short[keys[p.value[1]]] || keys[p.value[1]]}<br/>得分率: <b>${(p.value[2] * 100).toFixed(1)}%</b>` }, grid: { left: '12%', right: '8%', top: '5%', bottom: '5%' }, xAxis: { type: 'category', data: [...areas], axisLabel: { color: '#383874', fontSize: 12, fontWeight: 'bold' as const } }, yAxis: { type: 'category', data: keys.map(k => short[k] || k), axisLabel: { color: '#9292C1', fontSize: 11 } }, visualMap: { min: 0.4, max: 1, orient: 'vertical', right: 0, top: 'center', inRange: { color: ['#FF708B', '#FFBA69', '#66C8FF', '#8676FF'] }, textStyle: { color: '#9292C1', fontSize: 10 } }, series: [{ type: 'heatmap', data: heatData, label: { show: true, fontSize: 12, fontWeight: 'bold' as const, color: '#fff', formatter: (p: any) => (p.value[2] * 100).toFixed(1) + '%' }, itemStyle: { borderRadius: 3, borderWidth: 2, borderColor: '#fff' } }] });
    return () => chart.dispose();
  }, [data]);
  ...
  useEffect(() => {
    ...
    const indCatColors: Record<string, string> = { 'A类-学校管理与安全': '#00B929', 'B类-办学硬件与环境': '#FFBA69', 'C类-师资队伍与发展': '#8676FF' };
    const chart = mountEcharts(scatterRef.current, { tooltip: { trigger: 'item', formatter: (p: any) => `${p.data.area}<br/>${p.data.indicator}<br/>得分率: <b>${(p.data.value[1] * 100).toFixed(1)}%</b>` }, grid: { left: '8%', right: '4%', top: '10%', bottom: '10%' }, xAxis: { type: 'category', data: [...areas], axisLabel: { color: '#383874', fontSize: 11 }, splitLine: { lineStyle: { color: '#F2F5FA' } } }, yAxis: { type: 'value', max: 1, name: '得分率', nameTextStyle: { fontSize: 10 }, axisLabel: { color: '#9292C1', fontSize: 10, formatter: (v: number) => (v * 100).toFixed(0) + '%' }, splitLine: { lineStyle: { color: '#F2F5FA' } } }, series: areas.map((a, i) => ({ type: 'scatter', name: a, data: pts.filter(p => p[2] === a).map(p => {
      const cat = data.indicators.find(ind => ind.name === p[3])?.category || '';
      return { value: [i, p[1]], area: p[2], indicator: p[3], name: p[3], itemStyle: { color: indCatColors[cat] || areaColors[i] + '66', borderColor: indCatColors[cat] || areaColors[i], borderWidth: 0.5 } };
    }), symbolSize: 7, itemStyle: { borderWidth: 0.5 } })) });
    return () => chart.dispose();
  }, [data]);
